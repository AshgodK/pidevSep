...
class EvennementController extends AbstractController
{
    ...
    public function updateNbreParticipant(Request $request, Evennement $event, MailerInterface $mailer, PasseRepository $passeRepository, ManagerRegistry $doctrine): Response
    {
        ...
        if ($nbrP && $p) {
            ...
            $event->setNbrP($nbrP - 1);
            $entityManager->persist($event);
            $entityManager->flush();


            $passe = new Passe($p, $event, $this->getUser());
            ...
            $email = (new TemplatedEmail())
                ...
                ->subject('passe generer ')
                ->htmlTemplate('mailpasse.html.twig');

            $mailer->send($email);
            return $this->redirectToRoute('app_evennement_front_show', ['id' => $event->getId()]);
        }

        return $this->redirectToRoute('app_evennement_front_indexd');
    }
...
*/
    ...
    {
        $email = (new TemplatedEmail())
            ->from(new Address('achref.essid@esprit.tn', 'UltraHealth Bot'))
            ->to("mailjavam@gmail.com")
            ->subject('passe generer ')
            ->htmlTemplate('mailTemplate.html.twig');

        $mailer->send($email);
        return $this->redirectToRoute('app_evennement_front_index');
        // ...
    }
}
